...
def loose_game(answer_new, reply_new, all_repl, voc):
        all_repl.append(answer_new.get())
...
def cities_game():
    ...
    root["bg"] = 'pink'
    eliza_say = Label(root, text='Давайте поиграем в Города! Я хожу: Лондон',
                              wraplength='500', width = '70', bg = 'pink')
    eliza_say.grid(column = 0, row = 1)
    answer_new = Entry(root, width = 70)
    answer_new.grid(column = 1, row = 0)
    all_replies = ['Лондон']
    all_replies.append(answer_new.get())
